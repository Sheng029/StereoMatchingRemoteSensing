...
from stereonet_config import L2, alpha


class Conv3x3(models.Model):
    """
...
class Conv5x5BnRelu(models.Model):
    """
    Conv5x5 followed by bn and relu.
    """
    def __init__(self, filters, strides):
        super(Conv5x5BnRelu, self).__init__()
        self.conv = layers.Conv2D(filters=filters, kernel_size=5, strides=strides, padding='same',
...
class ResidualBlock(models.Model):
    ...
    def __init__(self, filters, dilation_rate):
        ...
        self.conv1 = layers.Conv2D(filters=filters, kernel_size=3, strides=1, padding='same',
                                   dilation_rate=dilation_rate, activation=None,
                                   use_bias=False, kernel_initializer='he_normal',
                                   kernel_regularizer=keras.regularizers.l2(L2))
        self.bn1 = layers.BatchNormalization()
        self.leaky_relu1 = layers.LeakyReLU(alpha=alpha)
        ...
        self.leaky_relu2 = layers.LeakyReLU(alpha=alpha)
