...
class FeatureExtractor(keras.Model):
    ...
    def __init__(self, filters):
        ...
        self.conv0_2 = conv2d_bn(filters, 3, 2, 'same', 1, True)
        self.conv0_3 = conv2d_bn(filters, 3, 1, 'same', 1, True)

        self.conv1_x = make_blocks(filters, 1, 12)
        self.conv2_x = make_blocks(filters, 2, 3)
        self.conv3_x = make_blocks(filters, 4, 3)

        self.branch_1 = avg_pool(32, 32, filters)
        self.branch_2 = avg_pool(16, 16, filters)
        self.branch_3 = avg_pool(8, 8, filters)
        self.branch_4 = avg_pool(4, 4, filters)
    ...
    def call(self, inputs, training=None, mask=None):
        outputs = self.conv0_1(inputs)
        outputs = self.conv0_2(outputs)
        outputs = self.conv0_3(outputs)

        outputs1_x = self.conv1_x(outputs)
        outputs2_x = self.conv2_x(outputs1_x)
        outputs3_x = self.conv3_x(outputs2_x)

        out_branch_1 = self.branch_1(outputs3_x)
        out_branch_1 = tf.image.resize(out_branch_1, [outputs3_x.shape[1], outputs3_x.shape[2]])

        out_branch_2 = self.branch_2(outputs3_x)
        out_branch_2 = tf.image.resize(out_branch_2, [outputs3_x.shape[1], outputs3_x.shape[2]])

        out_branch_3 = self.branch_3(outputs3_x)
        out_branch_3 = tf.image.resize(out_branch_3, [outputs3_x.shape[1], outputs3_x.shape[2]])

        out_branch_4 = self.branch_4(outputs3_x)
        out_branch_4 = tf.image.resize(out_branch_4, [outputs3_x.shape[1], outputs3_x.shape[2]])

        outputs_feature = tf.concat([outputs1_x, outputs3_x, out_branch_1, out_branch_2, out_branch_3, out_branch_4], -1)
        outputs_feature = self.fusion(outputs_feature)
