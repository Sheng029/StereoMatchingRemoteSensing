...
from features import LowScaleFeatureExtractor
...
class LowScaleModel:
    ...
    def build_model(self):
        ...
        feature_extractor = LowScaleFeatureExtractor(filters=16)
        left_feature = feature_extractor(left_image)
        right_feature = feature_extractor(right_image)

        cost_difference = CostDifference(max_disp=self.max_disp // 8)
        ...
        computation = DisparityComputation(max_disp=self.max_disp // 8)
        disparity = computation(agg_cost_volume)   # 1/8

        refine = Refinement(filters=16)
        refined_disparity = refine([disparity, left_image])   # 1/2

        d0 = tf.image.resize(disparity, [self.height, self.width]) * 8
